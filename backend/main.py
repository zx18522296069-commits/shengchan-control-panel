...
from .github_client import run_parts, run_split
...
def authorize(x_control_key: str = Header(default="")):
    ...
    if not expected:
        raise HTTPException(status_code=503, detail="CONTROL_PANEL_KEY 未配置")
    if not hmac.compare_digest(x_control_key, expected):
        raise HTTPException(status_code=401, detail="控制台口令不正确")
...
def update_config(payload: dict):
    try:
        updated, commits = save_config(payload)
    except ValueError as error:
        raise HTTPException(status_code=422, detail=str(error)) from error
    except requests.RequestException as error:
        raise HTTPException(status_code=502, detail=f"GitHub 同步失败：{error}") from error
